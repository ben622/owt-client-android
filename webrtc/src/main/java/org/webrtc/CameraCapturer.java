  ...
  public void switchCamera(final CameraSwitchHandler switchEventsHandler) {
    ...
    cameraThreadHandler.post(new Runnable() {
      ...
      public void run() {
        ...
        String cameraId = deviceNames.get((cameraNameIndex + 1) % deviceNames.size());

        //只在前后摄像头中切换，以解决在多个摄像头环境下可能导致切换无效的问题.
        boolean frontFacing = cameraEnumerator.isFrontFacing(cameraName);
        for (String deviceName : deviceNames) {
          if (frontFacing && cameraEnumerator.isBackFacing(deviceName)) {
            cameraId = deviceName;
            break;
          } else if (cameraEnumerator.isFrontFacing(deviceName)) {
            cameraId = deviceName;
            break;
          }
        }

        switchCameraInternal(switchEventsHandler, cameraId);
  }
});
  }
